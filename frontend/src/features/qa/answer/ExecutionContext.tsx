...
export function ExecutionContext({ intent, normalizedQuestion, round, maxRounds }: ExecutionContextProps) {
  return (
    <div className="clarification-context" aria-label="问题识别结果">
      <div><span>识别意图</span><Tag>{intent ? intentLabels[intent] ?? '未知意图' : '待确认'}</Tag></div>
      {normalizedQuestion ? <div><span>规范问题</span><strong>{normalizedQuestion}</strong></div> : null}
      <div><span>补充轮次</span><strong>第 {round} / {maxRounds} 轮</strong></div>
    </div>
  );
}
